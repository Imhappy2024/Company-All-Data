...
const query = (text, params) => db.q(text, params);
...
const channelOf = type => CHANNEL_BY_TYPE[String(type || '').toUpperCase()] || 'other';
...
/* ---- brand scoping -------------------------------------------------------
   ...
   The portal is a brand-as-workspace shell, so the LeavenWealth workspace must
   show LeavenWealth's GHL sub-accounts and nobody else's. `ghl_location`
   already carries `company_id` pointing at the same four-brand `company` table
   the portal's BRAND_COMPANY_ID map uses, so the scope is a column filter
   rather than anything new.

   It is applied HERE and in allowedLocationIds(), and that pairing is the
   whole security boundary: every other read in this file takes a set of
   location ids and trusts it. Scope the sidebar alone and the list looks
   right while /leads/:id and /thread still answer for any location anyone
   guesses.

   A location with a NULL company_id belongs to no brand and appears only in
   the unscoped (Executive) view. Silently attributing it to whichever brand is
   open would put another business's leads on the screen — the one thing this
   change exists to prevent. */
async function subAccounts(onlyId = null, companyId = null){
  const brand = await canJoinBrand();
  /* No company column means the scope cannot be enforced. Returning
     everything would leak across brands, so it returns nothing and the caller
     reports why — see ghl-api.js. */
  if (companyId && !brand) return [];
  const { rows } = await query(
    `SELECT gl.ghl_location_id AS id,
      ...
      WHERE ($1::text IS NULL OR gl.ghl_location_id = $1)
        ${brand ? 'AND ($2::uuid IS NULL OR gl.company_id = $2)' : ''}
      ORDER BY 2`,
    brand ? [onlyId, companyId] : [onlyId]);
...
/* Ids only. subAccounts() counts every lead and opportunity per location, which
   ...
   every request. */
/* The gate. thread, detail and every per-lead read check membership here, so
   scoping this scopes all of them — which is why the brand filter lives in
   this function rather than only in the list the sidebar renders. */
async function allowedLocationIds(companyId = null){
  if (!companyId) {
    const { rows } = await query(`SELECT ghl_location_id FROM ghl_location`);
    return new Set(rows.map(r => r.ghl_location_id));
  }
  if (!(await canJoinBrand())) return new Set();   /* cannot scope, so allow nothing */
  const { rows } = await query(
    `SELECT ghl_location_id FROM ghl_location WHERE company_id = $1::uuid`, [companyId]);
  return new Set(rows.map(r => r.ghl_location_id));
}
...
/* companyId widens the scope to leads that know their BRAND but not their GHL
   location. Verified 2026-09-07: 33 such rows exist — Leadli 18, Liquid 8,
   LeavenWealth 5, Folio 2 — and the location-only predicate command-center
   shipped hides every one of them from every screen. All 8 of Liquid Lending's
   leads are in that set, so without this the brand reads "no sub-account" while
   holding real leads.

   They have no messages and no opportunities by construction (both are keyed by
   location), so they list with contact detail and nothing else. */
async function leadRows(ids, limit, { since = null, contactId = null, search = null, companyId = null } = {}){
  ...
  const { rows } = await query(
    `WITH msg AS (
     ...
     SELECT * FROM (
      ...
      LIMIT $2`,
    [ids, limit, since, contactId, like, digitLike, companyId]);
  return rows;
}
...
async function loadLead(locationId, { contactId, opportunityId }){
  if (contactId) {
    const { rows } = await query(
...
async function threadFor(locationId, contactId, limit = 1000){
  const { rows } = await query(
...
async function pendingInbound(locationId, contactId){
  const { rows } = await query(
...
async function conversationsFor(locationId, contactId){
  const { rows } = await query(
...
async function stagesFor(locationId, pipelineId){
...
async function stageCounts(locationId){
...
async function customValuesFor(contactId){
  const { rows } = await query(
...
async function attributionFor(contactId){
  const { rows } = await query(
...
async function appointmentsFor(contactId){
  const { rows } = await query(
...
async function locationProfile(locationId){
  const { rows } = await query(
...
module.exports = { canJoinBrand, subAccounts, allowedLocationIds, locationIds, leadRows, leadTotal, loadLead, threadFor, pendingInbound, conversationsFor, stagesFor, stageCounts, customValuesFor, attributionFor, notesFor, tasksFor, appointmentsFor, ingestStatus, locationProfile, ACTIVITY_PREFIX, NOT_ACTIVITY, isActivity, dirOf, CHANNEL_BY_TYPE, channelOf, ACTIVITY_LABELS, activityLabel, PALETTE };
