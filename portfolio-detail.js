...
const ghlClient = () => db.getPool().connect();
...
const GROUP_ORDER = ['Property', 'Amenities & Systems', 'Loan & Debt', 'Insurance',
...
function propertyDetailRoutes({ invalidate }){
  const r = express.Router();
  const configured = db.enabled;
  ...
  r.get('/:id/detail', guard, async (req, res) => {
    const id = String(req.params.id);
    try {
      await colTypes();
      const one = async (sql, params) => (await db.q(sql, params)).rows;
  ...
  r.patch('/:id/field', guard, express.json({ limit: '128kb' }),
    async (req, res) => {
  ...
  r.post('/:id/owners', guard, express.json({ limit: '16kb' }),
    async (req, res) => {
      const id = String(req.params.id);
  ...
  r.delete('/:id/owners/:entityId', guard, async (req, res) => {
    ...
    try {
      ...
      await db.q(`delete from public.ownership where id = $1`, [row.id]);
      invalidate();
      res.json({ ok: true });
    } catch (err) { fail(res, err); }
  });
  ...
  r.get('/:id/comments', guard, async (req, res) => {
    try {
      const rows = (await db.q(
  ...
  r.post('/:id/comments', guard, express.json({ limit: '32kb' }),
    async (req, res) => {
      ...
      try {
        ...
        const author = req.session?.user?.email || req.session?.user?.name
          || process.env.OWNER_EMAIL || 'LeavenWealth Portal';
        const [row] = (await db.q(
  ...
  r.post('/entity', guard, express.json({ limit: '16kb' }),
    async (req, res) => {
  ...
  r.post('/property', guard, express.json({ limit: '16kb' }),
    async (req, res) => {
      const b = req.body || {};
  ...
  r.post('/:id/building', guard, express.json({ limit: '16kb' }),
    async (req, res) => {
      const name = String(req.body?.name || '').trim();
...
module.exports = { propertyDetailRoutes, GROUP_ORDER };
